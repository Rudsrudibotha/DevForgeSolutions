...
function statusForPayslipError(error, fallbackStatus) {
  return /permission/i.test(error.message) ? 403 : fallbackStatus;
}
...
), audit('Payslip', 'ListAll'), async (req, res) => {
  try {
    const options = {
    ...
    res.status(statusForPayslipError(error, 500)).json({ error: error.message });
  }
});

router.get('/previous', authenticateToken, requireSchoolPermission('payroll.view_previous', 'hr.view_payslips', 'hr.manage_payslips', 'sensitive.payroll.view'), audit('Payslip', 'ListPrevious'), async (req, res) => {
  try {
...
router.get('/:id', authenticateToken, requireSchoolOrAdmin, audit('Payslip', 'ViewSingle'), async (req, res) => {
  try {
    ...
    const status = /permission/i.test(error.message) || error.message.includes('not enabled') ? 403 : 404;
...
router.post('/', authenticateToken, requireSchoolPermission('hr.manage_payslips', 'payroll.generate'), audit('Payslip', 'Create'), async (req, res) => {
  try {
    ...
    res.status(statusForPayslipError(error, 400)).json({ error: error.message });
  }
});
...
router.post('/:id/export-audit', authenticateToken, requireSchoolPermission(
  'hr.view_payslips',
  'hr.manage_payslips',
  'payroll.generate',
  'payroll.review',
  ...
  'sensitive.payroll.view'
), audit('Payslip', 'Export'), async (req, res) => {
  try {
    const payslip = await payslipService.getPayslipById(parseInt(req.params.id, 10), req.user);
    res.json({ PayslipID: payslip.PayslipID, SchoolID: payslip.SchoolID, exported: true });
  } catch (error) {
    res.status(statusForPayslipError(error, 404)).json({ error: error.message });
  }
});
