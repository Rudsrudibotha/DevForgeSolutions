...
const { authenticateToken, requireSchoolOrAdmin, requireSchoolPermission } = require('../middleware/auth');

const router = express.Router();
const leaveService = new LeaveService();

// Get all leave requests for the school/admin
router.get('/', authenticateToken, requireSchoolPermission('leave.view_all', 'leave.approve', 'leave.decline'), async (req, res) => {
  try {
...
router.get('/mine', authenticateToken, requireSchoolOrAdmin, async (req, res) => {
  try {
...
router.get('/:id', authenticateToken, requireSchoolPermission('leave.view_all', 'leave.approve', 'leave.decline'), async (req, res) => {
  try {
...
router.put('/:id/review', authenticateToken, requireSchoolPermission('leave.approve', 'leave.decline'), async (req, res) => {
  try {
